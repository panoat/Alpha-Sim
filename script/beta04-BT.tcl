...
set BP_MODE     4       ;# broadcast tree probing mode
...
Agent/AODV set bct_mode_                $val(bct_mode)
...
# Setup broadcast tree by sending a packet in AF mode to establish parent_ip in each node
if { $val(bct_mode) == $BCT_ENABLE } {
    $ns_ at [expr $val(start) * 0.1] "$dummy_rt fwd-mode $BP_MODE"
    $ns_ at [expr $val(start) * 0.1] "$dummy_rt bct-enable"
